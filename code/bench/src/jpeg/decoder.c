...
static uint8_t  read_byte(uint8_t** fp);
static uint16_t read_uint(uint8_t** fp);
static bool eof(JPG_t* jpg);
static void init_jpg(JPG_t* jpg);
static void ReadAPPn(uint8_t** fp, JPG_t* jpg);
static void ReadDQT(uint8_t** fp, JPG_t* jpg);
static void ReadSOF0(uint8_t** fp, JPG_t* jpg);
static void ReadDRI(uint8_t** fp, JPG_t* jpg);
static void ReadDHT(uint8_t** fp, JPG_t* jpg);
static void ReadSOS(uint8_t** fp, JPG_t* jpg);
static void ReadComment(uint8_t** fp, JPG_t* jpg);
 ...
 * */
bool JPG_decode( JPG_t* jpg, uint8_t* fp, uint8_t* fb)
{
  uint8_t byte0;
  uint8_t byte1;
  ...
  {
    #if (_DEBUG_PRINT > 0)
    printf("ERROR: byte0 = %02x expected 0xff, byte1 = %02x expected 0xd8\n", byte0, byte1);
    #endif
    jpg->valid = false;
    return false;
  }
...
static uint16_t read_uint(uint8_t** fp)
{
  uint16_t b0 = read_byte(fp);
  uint16_t b1 = read_byte(fp);
    ...
    for(int j = 0; j < jpg->horizontalSamplingFactor; j++)
    {
      for(int y = 0; y < 8; y++)
      {
        for(int x = 0; x < 8; x++)
        {
          ...
          int index = (img_y*jpg->width + img_x)*3;
          int r, g, b;
          ...
          if(img_x < jpg->width && img_y < jpg->heigth)
          {
            ...
            jpg->frame_buffer[index]   = r;
            jpg->frame_buffer[index+1] = g;
            jpg->frame_buffer[index+2] = b;
          }
        }
      }
    }
